...
async function run() {
    ...
    if (!validateBranchName({branchName: baseBranch})) {
        core.setFailed('Invalid base-branch name. Branch names should only include characters, numbers, hyphens, underscores, dots, and foreward slashes.');
        return;

    }

    if (!validateBranchName({branchName: targetBranch})) {
        core.setFailed('Invalid target-branch name. Branch names should only include characters, numbers, hyphens, underscores, dots, and foreward slashes.');
        return;
    }

    if (!validateDirectoryName({dirName: workingDir})) {
        core.setFailed('Invalid working-directory name. Directory names should only include characters, numbers, hyphens, underscores, and foreward slashes.');
        return;
    }
    ...
    if (gitStatus.stdout.length > 0) {
        ...
        core.info('[js-dependency-update] : No updates available at this time.')
    }
